...
static void tri_sort(int a,int b,int c){
    if (a>=n) return;
    if (b>=n) return;
    h1=a;h2=b;
    cmp_swap(a,b);
...
static int cyriax_sorting_network(){
    CONTINUE(1);
    CONTINUE(2);
    CONTINUE(3);
    for (a=0;a<n;a++){
        //if (data[a] > n/2) data[a]=n/3;
        //else data[a] = n*2/3;
    }

    for (step = 1;step < n;step *= 3){
        for (j=0;j<n;j+=3*step){
            for (i=j;(i<j+step)&&(i<n);i++){
                printf(" %d<%d<%d \n",i,i+step,i+2*step);
                tri_sort(i,i+step,i+2*step);
                YIELD(3);
            }
        }
    }
    //basically cheat here
    for (start=0;start<3;start++){
        for (i=start+3;i<n;i+=3){
            for (j=i; j-3>=0&&data[j-3]>data[j]; j-=3) {
                cmp_swap(j-3,j);
            }
        }
    }
    ...
    for (step=1;step*3<n;step*=3);
    for (;step>1;step/=3){
        for (j=1;j<n;j*=3){
            for (i=0;i<n;i+=3*j){
                tri_sort(i+2,i+step+1,i+2*step);
                YIELD(1);
                tri_sort(i,i+1,i+2);
                tri_sort(i+step,i+step+1,i+step+2);
                tri_sort(i+2*step,i+2*step+1,i+2*step+2);
            }
        }
    }
    for(i=1;i+3<n;i+=3){
...
// Modified median of 3 introsort
static int cyriax_quacksort(){
    CONTINUE(1);
    CONTINUE(2);
    data2[0] = 0;
    data2[1] = n-1;
    ...
    while(stack_p > 0){
        ...
        rr = data2[stack_p-1];
        stack_p -= 2;
        if (rr - ll <= 1 << (logn/2)) continue;
        // partition
        cmp_swap(ll,rr);
        for(h1=a=l=ll,h2=b=r=rr;l<r-1;){
            h1=l;h2=r;
            YIELD(1);
            if (data[r] < data[r-1]){
                cmp_swap(r-1,r);
...
static int cyriax_whatever(){
    CONTINUE(1);
    for (stage=1;stage<n;stage++){
        for (j=0;j<n;j+=n/stage){
            a = j+(n/stage)-1;
            for (i=0;j+i<a-i;i++){
                if (a-i >= n) continue;
                cmp_swap(j+i,a-i);
...
static int sort() {
    ...
    case 18: return cyriax_sorting_network();
    case 19: return cyriax_quacksort();
    case 20: return cyriax_whatever();
